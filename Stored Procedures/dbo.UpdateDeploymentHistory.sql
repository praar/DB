...
begin
	...
	using (select 
		...
		) as source(DeploymentId, DeploymentName, ProjectId, ProjectName, ProjectSlug, EnvironmentId, EnvironmentName, ReleaseId, ReleaseVersion, TaskId, TaskState, Created, QueueTime, StartTime, CompletedTime, DurationSeconds, DeployedBy)
	...
	when not matched then 
		INSERT (DeploymentId, DeploymentName, ProjectId, ProjectName, ProjectSlug, EnvironmentId, EnvironmentName, ReleaseId, ReleaseVersion, TaskId, TaskState, Created, QueueTime, StartTime, CompletedTime, DurationSeconds, DeployedBy) 
		VALUES (source.DeploymentId, source.DeploymentName, source.ProjectId, source.ProjectName, source.ProjectSlug, source.EnvironmentId, source.EnvironmentName, source.ReleaseId, source.ReleaseVersion, source.TaskId, source.TaskState, source.Created, source.QueueTime, source.StartTime, source.CompletedTime, source.DurationSeconds, source.DeployedBy)
	;
